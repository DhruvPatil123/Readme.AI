...
**Readme.AI** is an elite documentation engine designed to bridge the gap between raw source code and professional, high-impact project presentation. By leveraging advanced generative AI and deep repository analysis, it transforms complex codebases into structured, readable, and aesthetic documentation in seconds.
...
The application uses `@octokit/rest` to communicate with the GitHub API. It traverses the file tree, identifying key files (`package.json`, `.env`, `src/`) to create a context-rich metadata object.

### 2. Processing Layer (The "Brain")
The Express server (`server.ts`) acts as a middleware between the client and Google's Gemini Pro. It constructs a complex prompt containing the code structure and key file contents, instructing the AI to output structured Markdown.

### 3. Presentation Layer
Built with Vite and React, the frontend utilizes a reactive state to manage the lifecycle of the documentation generation. `react-markdown` ensures that the AI's output is rendered instantly with high fidelity.
